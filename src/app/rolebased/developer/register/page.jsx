...
function page() {
    ...
    const developerRegister = async(e) => {
      e.preventDefault()
      try {
        const response = await axios.post("/api/developer/register",user)
        console.log(response)
        toast.success("Registered successfully")
        router.push("/rolebased/developer/login")
      } catch (error) {
        console.log(error.message,"messsssssssssssssssss")
        toast.error("Something went wrong")
      }
    }
  return (
    <div className='h-screen bg-white text-center flex items-center justify-end'>
    <img src="https://uploads-ssl.webflow.com/5a4347c1115b2f0001333231/5a436d6080557a000178b2a6_Exterior002-p-1080.jpeg" alt="" className='w-full h-full object-cover' />
    <div className='absolute md:w-[40%] p-5'>
       ...
       <div className='flex items-center gap-5 justify-center'>
            ...
            <div>
                <button className='bg-gray-900 text-white rounded-md p-2 w-full mt-5 font-bold' onClick={developerRegister}>
                {loading ? <BeatLoader color='white' /> : "Register"}
                </button>
            </div>
            <div className=' mt-5'>
                <p className='text-black font-bold underline cursor-pointer'>
                    Already have an account? <Link href='/rolebased/developer/login'><span className='font-bold text-black'>Login</span></Link>
                </p>
            </div>
       </div>
    </div>
